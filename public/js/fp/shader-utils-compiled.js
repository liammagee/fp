...
define(["fp/fp-base"], function (FiercePlanet) {
    ...
    FiercePlanet.ShaderUtils = {
        ...
        lambertShaderFragment: function lambertShaderFragment(customParams, customCode) {
            ...
            THREE.ShaderChunk["logdepthbuf_fragment"], THREE.ShaderChunk["map_fragment"], THREE.ShaderChunk["color_fragment"], THREE.ShaderChunk["alphamap_fragment"], THREE.ShaderChunk["alphatest_fragment"], THREE.ShaderChunk["specularmap_fragment"], THREE.ShaderChunk["shadowmap_fragment"], "\n                        #ifdef DOUBLE_SIDED\n\n                           if ( gl_FrontFacing )\n                               outgoingLight += diffuseColor.rgb * ( vLightFront * shadowMask ) + emissive;\n                           else\n                               outgoingLight += diffuseColor.rgb * ( vLightBack * shadowMask ) + emissive;\n\n                        #else\n\n                           outgoingLight += diffuseColor.rgb * ( vLightFront * shadowMask ) + emissive;\n\n                        #endif\n                        ", THREE.ShaderChunk["envmap_fragment"], THREE.ShaderChunk["linear_to_gamma_fragment"], THREE.ShaderChunk["fog_fragment"], "\n                            gl_FragColor = vec4( outgoingLight, diffuseColor.a ); // TODO, this should be pre-multiplied to allow for bright highlights on very transparent objects\n                        }\n                        "].join("\n");
        ...
        phongShaderFragment: function phongShaderFragment(customParams, customCode) {
            ...
            "   vec4 diffuseColor = vec4( diffuse, opacity );", "   ReflectedLight reflectedLight = ReflectedLight( vec3( 0.0 ), vec3( 0.0 ), vec3( 0.0 ), vec3( 0.0 ) );", "   vec3 totalEmissiveLight = emissive;", "   vec3 shadowMask = vec3( 1.0 );", THREE.ShaderChunk["logdepthbuf_fragment"], THREE.ShaderChunk["map_fragment"], THREE.ShaderChunk["color_fragment"],
            // Original Location in three.js ShaderLib.js
            // THREE.ShaderChunk[ "alphamap_fragment" ],
            // THREE.ShaderChunk[ "alphatest_fragment" ],
            THREE.ShaderChunk["specularmap_fragment"], THREE.ShaderChunk["normal_phong_fragment"], THREE.ShaderChunk["emissivemap_fragment"], THREE.ShaderChunk["lights_phong_fragment"], THREE.ShaderChunk["lights_template"], THREE.ShaderChunk["lightmap_fragment"], THREE.ShaderChunk["aomap_fragment"], THREE.ShaderChunk["shadowmap_fragment"], "reflectedLight.directDiffuse *= shadowMask;", "reflectedLight.directSpecular *= shadowMask;", "vec3 outgoingLight = reflectedLight.directDiffuse + reflectedLight.indirectDiffuse + reflectedLight.directSpecular + reflectedLight.indirectSpecular + totalEmissiveLight;", THREE.ShaderChunk["envmap_fragment"], THREE.ShaderChunk["linear_to_gamma_fragment"], THREE.ShaderChunk["fog_fragment"], customCode, // must set gl_FragColor!
